...
Page({
  data: {
    ...
    wolfTargetIndex: 0, guardTargetIndex: 0, seerTargetIndex: 0, witchTargetIndex: 0,
    witchActions: ['不用药', '使用解药', '使用毒药'], witchActionIndex: 0,
    inspection: '', wolfMembers: '', seerNumber: '', witchNumber: '', guardNumber: '', wolfAlive: false, guardAlive: false, seerAlive: false, witchAlive: false, hasGuard: false,
    ...
    voteRecords: [], voteTally: [], sheriffWeight: 1.5, sheriffSeat: null, selfExposeCandidates: [], pendingWolfKingClaw: false, pendingSkill: null, pendingLastWord: null, pendingSkillCanTarget: false, choosingSkillTarget: false, skillTargetLabels: [], selectedSkillTargetIndex: 0, simpleVoteCount: '', dayMessage: '', seatCards: [], voteHistory: [], selectedVoteHistoryIndex: 0, selectedVoteRound: null, selectedVoteRows: [], seatInfo: null
  },
  ...
  refresh() {
    ...
    const seer = roleSeat('seer'); const witch = roleSeat('witch'); const guard = roleSeat('guard')
    ...
    this.setData({
      ...
      wolfTargetIndex: indexFor(game.night.wolfTarget), guardTargetIndex: indexFor(game.night.guardTarget), seerTargetIndex: indexFor(game.night.seerTarget), witchTargetIndex: indexFor(game.night.witchTarget), witchActionIndex: ['none', 'save', 'poison'].indexOf(game.night.witchAction),
      ...
      seerNumber: seer ? seer.number : '', witchNumber: witch ? witch.number : '', guardNumber: guard ? guard.number : '',
      wolfAlive: game.seats.some(seat => seat.alive && roles[seat.roleId].camp === 'wolf'), guardAlive: engine.roleAlive(game, 'guard'), seerAlive: engine.roleAlive(game, 'seer'), witchAlive: engine.roleAlive(game, 'witch'), hasGuard: Boolean(guard),
      ...
      seatCards, voteHistory, selectedVoteHistoryIndex: voteIndex, selectedVoteRound, selectedVoteRows,
      ...dayView
    })
  },
  ...
  finishNight() { const game = this.data.game; if (game.night.witchAction === 'save') game.resources.antidote = false; if (game.night.witchAction === 'poison') game.resources.poison = false; engine.settleNight(game); this.saveAndRefresh(game) },

  declineDeathSkill() { const game = this.data.game; engine.resolveDeathSkill(game, this.data.pendingSkill.number, false); this.afterDeathSkill(game) },
  requestDeathSkill() {
    if (this.data.pendingSkillCanTarget) return this.setData({ choosingSkillTarget: true })
    const game = this.data.game; engine.resolveDeathSkill(game, this.data.pendingSkill.number, true); this.afterDeathSkill(game)
  },
  selectSkillTarget(event) { this.setData({ selectedSkillTargetIndex: Number(event.detail.value) }) },
  confirmDeathSkillTarget() { const game = this.data.game; const target = this.data.aliveSeats[this.data.selectedSkillTargetIndex]; if (!target) return this.toast('没有可选择的目标'); engine.resolveDeathSkill(game, this.data.pendingSkill.number, true, target.number); this.afterDeathSkill(game) },
  ...
  nextLastWord() { const game = this.data.game; game.pendingLastWords.shift(); if (!game.pendingLastWords.length) game.dayState.stage = game.dayState.endAfterDeathSkills ? 'selfExposeNight' : 'discussion'; this.saveAndRefresh(game) },
  ...
  beginSheriffSpeech() { const game = this.data.game; if (!game.dayState.sheriffCandidates.length) return this.toast('请先记录上警玩家'); game.dayState.stage = 'sheriffSpeech'; engine.addLog(game, `第${game.day}天：上警玩家为${game.dayState.sheriffCandidates.map(n => `${n}号`).join('、')}`); this.saveAndRefresh(game) },
  returnSheriffSignup() { const game = this.data.game; game.dayState.stage = 'sheriffSignup'; this.saveAndRefresh(game) },
  withdrawSheriffCandidate(event) { const game = this.data.game; const number = Number(event.currentTarget.dataset.number); const state = game.dayState; if (!state.sheriffCandidates.includes(number)) return; state.sheriffCandidates = state.sheriffCandidates.filter(item => item !== number); if (!state.sheriffWithdrawn.includes(number)) state.sheriffWithdrawn.push(number); this.saveAndRefresh(game) },
  beginSheriffVote() { const game = this.data.game; game.dayState.stage = 'sheriffVote'; this.saveAndRefresh(game) },
  ...
  resolveSheriffVote() {
    ...
    game.sheriffSeat = winners[0]; game.sheriffElectionDone = true; game.sheriffElectionInterrupted = false; game.dayState.stage = 'sheriffOrder'; engine.addLog(game, `第${game.day}天：${winners[0]}号当选警长`); this.saveAndRefresh(game)
  },
  restartSheriffVote() { const game = this.data.game; game.dayState.sheriffVotes = {}; game.dayState.stage = 'sheriffSpeech'; game.dayState.message = ''; this.saveAndRefresh(game) },
  noSheriff() { const game = this.data.game; game.sheriffElectionDone = true; game.dayState.stage = 'announceNight'; engine.addLog(game, `第${game.day}天：警长竞选平票，警徽流失`); this.saveAndRefresh(game) },
  setSpeakingOrder(event) { const game = this.data.game; const direction = event.currentTarget.dataset.direction; game.dayState.stage = 'announceNight'; game.dayState.speakingOrder = direction; engine.addLog(game, `第${game.day}天：警长决定${direction === 'clockwise' ? '顺序' : '逆序'}发言`); this.saveAndRefresh(game) },
  announceNightDeaths() { const game = this.data.game; engine.addLog(game, `第${game.day}天：${this.nightDeathText(game)}`); game.dayState.stage = game.pendingDeathSkills.length ? 'deathSkills' : game.pendingLastWords && game.pendingLastWords.length ? 'lastWords' : (game.dayState.endAfterDeathSkills ? 'selfExposeNight' : 'discussion'); this.saveAndRefresh(game) },
  beginExileVote() { const game = this.data.game; if (game.dayState.stage === 'exileTie') { game.dayState.exileVotes = {}; game.dayState.simpleVoteCounts = {} } game.dayState.stage = 'exileVote'; this.saveAndRefresh(game) },
  ...
  recordNoExile() { const game = this.data.game; engine.applyDayEvent(game, 'none'); game.dayState.stage = 'exileDone'; this.saveAndRefresh(game) },
  triggerSelfExpose() {
  ...
  enterSelfExposeNight() { const game = this.data.game; engine.nextNight(game); this.saveAndRefresh(game) },
  triggerWolfKingClaw() {
    const targets = this.data.aliveSeats
    wx.showActionSheet({ itemList: targets.map(item => `${item.number}号`), success: result => { const game = this.data.game; engine.applyDayEvent(game, 'wolfKingClaw', targets[result.tapIndex].number); this.saveAndRefresh(game) } })
  },
  ...
  enterNight() { const game = this.data.game; engine.nextNight(game); this.saveAndRefresh(game) },
