...
const { nightActions: nightActionDefinitions } = require('../../data/night-actions')
...
Page({
  ...
  onLoad(query) {
    ...
    this.setData({ board, roleCards, nightActions: board.nightActions.map(id => nightActionDefinitions[id] ? nightActionDefinitions[id].name : id), detailSections: parseBoardDocument(boardDocuments[board.id]) })
